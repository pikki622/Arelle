...
def evaluate(xpCtx, varSet, derivedFact):
    ...
    for consisAsserRel in xpCtx.modelXbrl.relationshipSet(XbrlConst.consistencyAssertionFormula).toModelObject(varSet):
            ...
            if (
                ...
                and inputFact.context.isEqualTo(
                    ...
                    dimensionalAspectModel=(
                        varSet.aspectModel == "dimensional"
                    ),
                )
                and (
        ...
        if (
            not aspectMatchedInputFacts
            and isStrict
            and derivedFact.isNil
            or aspectMatchedInputFacts
            and not derivedFact.isNil
        ):
            isSatisfied = True
        elif (
            not aspectMatchedInputFacts
            and isStrict
            or len(aspectMatchedInputFacts) != 0
        ):
            isSatisfied = False
        else:
            if xpCtx.formulaOptions.traceVariableSetExpressionResult:
        ...
        for fact in aspectMatchedInputFacts:
            ...
            if fact.isNil:
                ...
                if (
                    ...
                    and xpCtx.formulaOptions.traceVariableSetExpressionResult
                ):
                    xpCtx.modelXbrl.info("formula:trace",
                ...
                elif hasAbsoluteAcceptanceRadius:
                    acceptance = consisAsser.evalRadius(xpCtx, derivedFact.vEqValue)
                    isSatisfied = (
                        fabs(derivedFact.vEqValue - fact.vEqValue)
                        <= fabs(acceptance)
                        if acceptance is not None
                        else None
                    )
